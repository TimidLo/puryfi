...
$(".batch_converter-file-selector").change(function (event) {
   ...
   options.then((res) => {
      if (
         $(this).attr("id") === "folder_batch_converter-file-selector" &&
         (!Object.hasOwn(res, "user") ||
            !(
               res.user.permissions.permission_batch_converter <=
      ...
      batch_files.forEach(function (file, index) {
         ...
         if (file["type"].split("/")[0] === "image") {
            file.arrayBuffer().then((buffer) => {
               ...
               let { type, ext } = checkMimeHeader(header, buffer);

               let cont = res.file_types.includes(ext);
               if (ext === "gif") {
                  cont =
                     cont ||
                     (res.gif_configuration != null &&
                        res.gif_configuration._thumbnails);
               }
               ...
               if (
                  cont ||
                  ["png", "jpg", "jpeg", "bmp", "webp", "avif", "gif"].includes(
                     ext
                  )
               ) {
                  ...
                  $("#batch_converter_total_images_count").html(
                     batch_files_filtered.length
                  );
                  $("#batch_converter_total_file_size").html(
                     formatBytes(batch_file_size)
                  );
               }
            });
         }
...
$("#batch_converter_start-label").click(function () {
   ...
   if (!batch_process_ongoing) {
      ...
      options.then(async (res) => {
         ...
         if (batch_files_filtered.length > i) {
            ...
            for (const elem of batch_files_filtered) {
               ...
               await convert_element(
                  batch_files_filtered,
                  i,
                  res,
                  function (index, file, blob, type) {
                     let t1 = performance.now();
                     let t_single = t1 - batch_processing_time_total;
                     batch_processing_time_total = t1;
                     progress_bar.style.width =
                        (index / batch_files_filtered.length) * 100 + "%";
                     $("#batch_converter_progress_text").html(
                        Math.ceil((index / batch_files_filtered.length) * 100) +
                           "%"
                     );
                     logger.log(
                        "info",
                     ...
                     let average_time =
                        (batch_processing_time_total -
                           batch_processing_time_start) /
                        index;
                     let est_dur = average_time * batch_files_filtered.length;
                     let est_left =
                        average_time * (batch_files_filtered.length - index);
                     $("#batch_converter_estimated_time").html(
                        msToTime(est_dur)
                     );
                     $("#batch_converter_estimated_time_left").html(
                        msToTime(est_left)
                     );
                     $("#batch_converter_estimated_time_average").html(
                        msToTime(average_time)
                     );
                     let filepath = file.webkitRelativePath || file.name;
                     if (file.type !== blob.type) {
                        filepath = filepath.replace(/\.gif$/, ".png");
                     }
      ...
      logger.log(
         "info",
         "Batch processing interrupted: Finishing current index."
      );
   }
});
/*
         ...
         image.onload = function () {
            try {
               ...
               fileReader.onload = function (e) {
                  ...
                  let blob = new Blob([uint8Array], { type: full_type });
                  let req;
                  if (
                     full_type === "image/gif" &&
                     config.gif_configuration._thumbnails
                  )
                     full_type = "image/png";

                  if (full_type === "image/gif") {
                     req = browser.runtime.sendMessage({
                        type: "CENSOR_GIF",
                        blob: blob,
                        file_type: full_type,
                        skip_filtering: true,
                  ...
                  req.then((res) => {
                     ...
                     progress(i, file, res.blob, full_type);
                     resolve();
                  });
               };
...
function batch_converter_store_zip(zip, callback) {
   logger.log(
      "info",
      "DONE in " +
         (batch_processing_time_total - batch_processing_time_start) +
         " milliseconds."
   );
...
function batch_converter_log(type, time_stamp, message) {
   ...
   const hms =
      time_stamp.getHours() +
      ":" +
      time_stamp.getMinutes() +
      ":" +
      time_stamp.getSeconds();
   ...
   $("#batch_converter_console").scrollTop(
      $("#batch_converter_console")[0].scrollHeight
   );
}
